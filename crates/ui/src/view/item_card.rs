//! The in-game-style item tooltip card and the per-listing hover-preview card:
//! one renders a parsed [`parser::Item`], the other the trade API's raw JSON.
...
// POE2 tags every socket-granted line `(rune)` regardless of what's socketed.
...
pub(super) fn item_card(ui: &mut egui::Ui, item: &Item, icon_url: Option<&str>) {
    ...
    framed(
        ...
        |ui| {
            ...
            // A foil unique gets a per-frame shimmering title; others a flat colour.
...
fn state_badges(ui: &mut egui::Ui, item: &Item) {
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        for flag in &item.flags {
            // The foil marker gets its own gold pill, not the generic flag pill.
...
/// Longest mod-stat line rendered inline; longer lines are ellipsised to one line.
const MOD_STAT_MAX_CHARS: usize = 64;

/// Tighter cap for the narrow hover-preview card so each mod stays one line.
...
fn render_mod(ui: &mut egui::Ui, m: &Modifier) {
    ...
    for (i, stat) in m.stats.iter().enumerate() {
        ...
        ui.horizontal(|ui| {
            ...
            // Slot/name/tier right-aligned on the first line so values keep a clean column.
            if i == 0 {
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
...
/// Shorten an over-long mod stat to ~`max` chars at a word boundary with an ellipsis.
...
/// Whether the item copied as a foil unique (a `"Foil …"` flag).
fn is_foil(item: &Item) -> bool {
    item.flags.iter().any(|f| f.starts_with("Foil"))
}

/// A slowly-cycling holographic colour for a foil title, given the egui clock (s).
...
/// A rounded card: rarity border plus an optional outer accent-stroke glow.
fn framed(
    ui: &mut egui::Ui,
    border: Color32,
...
/// Extract one mod line from a `*Mods` entry, accepting both object and string forms.
...
/// Flatten a trade `properties`/`requirements` array into cleaned `(name, value)`
/// pairs, dropping entries with no value.
...
/// Clearance above the cursor; must stay positive so the tooltip never covers the
/// row and flips its `contains_pointer()`, which would flicker the preview.
const PREVIEW_CURSOR_GAP: f32 = 12.0;

/// Show the item preview centred on and floating above the cursor.
        ...
        |ui| {
            ...
            ui.horizontal(|ui| {
                if let Some(icon) = &item.icon {
                    // Fit (not stretch) into a ≤48px box, preserving aspect ratio.
                    ui.add(
                        egui::Image::new(icon)
            ...
            if !item.mods.is_empty() {
                ...
                for (cat, text) in &item.mods {
                    // One ellipsised line per mod: a tall preview gets shoved over
                    // the cursor by `constrain`, flickering the row's hover.
...
/// Strip POE2 fetch-text markup: `[link|display]` → `display`, `[text]` → `text`.
...
mod tests {
    ...
    /// Headless render: confirms the mod stat text reaches the paint output.
    #[test]
    ...
    fn preview_extracts_properties_requirements_sockets_and_categorised_mods() {
        // Mirrors the live trade2 shape: object-form mods and `[link|display]` markup.
